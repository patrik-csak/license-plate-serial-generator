...
describe('Hawaii', () => {
  ...
  describe('Honolulu county', () => {
    ...
    testSerial('Should contain valid letters', honoluluCounty(), serial => {
      ...
      const validLetters = Bb26.range({ start: 'eaa', end: 'haa' })
        .concat(Bb26.range({ start: 'jaa', end: 'kaa' }))
        .concat(Bb26.range({ start: 'naa', end: 'oaa' }))
        .concat(Bb26.range({ start: 'paa', end: 'qaa' }))
        .concat(Bb26.range({ start: 'raa', end: 'ttu' }))
        .filter(
          string => 'hklm'.split('').every(letter => !string.includes(letter)))
        .map(toUpperCase)
